...
import { Award, FileText, Briefcase, Key, ShieldCheck, ArrowRight, Send, MessageSquare, Calculator, TrendingUp, BookOpen, Calendar, Sparkles } from 'lucide-react';
  ...
  return (
    <div className={`relative overflow-hidden shadow-lg border-b w-full max-w-full text-white py-6 sm:py-8 px-3 sm:px-4 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-6 sm:space-y-8">
          ...
          <div className={`${isIndependenceActive ? 'md:col-span-8 space-y-3' : 'space-y-3'}`}>
            ...
            <div className="pt-1 flex flex-wrap items-center justify-center md:justify-start gap-2">
              <a
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white rounded-full font-black text-xs shadow-md border border-sky-300/60 transition-transform transform hover:scale-105"
              >
                <Send className="w-3.5 h-3.5 text-white" />
                <span>{language === 'hi' ? '📲 टेलीग्राम चैनल (@pariksha_result_official)' : '📲 Telegram (@pariksha_result_official)'}</span>
                <span className="bg-sky-300 text-slate-900 text-[10px] font-black px-1.5 py-0.2 rounded-full uppercase">Instant</span>
              </a>
            </div>
        ...
        {/* Candidate Smart Utilities Ribbon */}
        <div className="bg-slate-900/80 backdrop-blur-md p-3 sm:p-4 rounded-2xl border border-white/10 shadow-xl max-w-6xl mx-auto space-y-2">
          <div className="flex items-center justify-between px-1">
            <span className="text-[11px] font-black text-amber-300 uppercase tracking-widest flex items-center gap-1.5">
              ...
              {language === 'hi' ? 'अभ्यर्थी उपयोगी टूल (Candidate Utilities)' : 'Sarkari Aspirant Smart Tools'}
            </span>
            <span className="text-[10px] bg-white/10 text-slate-300 font-bold px-2 py-0.5 rounded-full">1-Click Free Tools</span>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
            {/* 1. Photo & Signature Resizer Button */}
            <button
              type="button"
              onClick={onOpenPhotoResizer}
              className="p-2.5 bg-gradient-to-r from-teal-600/90 to-emerald-800/90 hover:from-teal-500 hover:to-emerald-700 text-white rounded-xl border border-teal-400/30 text-left transition-all shadow-sm hover:scale-[1.02] cursor-pointer flex items-center gap-2.5"
            >
              <div className="p-2 bg-teal-500/20 rounded-lg shrink-0">
                <Sparkles className="w-4 h-4 text-emerald-300 animate-pulse" />
              </div>
              <div className="min-w-0">
                <div className="text-xs font-black truncate">{language === 'hi' ? 'फोटो रीसाइज़र' : 'Photo Resizer'}</div>
                <div className="text-[10px] text-teal-200 truncate">{language === 'hi' ? '20-50 KB SSC/UPSC' : '20-50 KB SSC/UPSC'}</div>
              </div>
            </button>

            {/* 2. Rank Predictor (Bell Curve) Button */}
            <button
              type="button"
              onClick={onOpenRankPredictor}
              className="p-2.5 bg-gradient-to-r from-indigo-600/90 to-blue-800/90 hover:from-indigo-500 hover:to-blue-700 text-white rounded-xl border border-indigo-400/30 text-left transition-all shadow-sm hover:scale-[1.02] cursor-pointer flex items-center gap-2.5"
            >
              <div className="p-2 bg-indigo-500/20 rounded-lg shrink-0">
                <ShieldCheck className="w-4 h-4 text-indigo-300 animate-pulse" />
              </div>
              <div className="min-w-0">
                <div className="text-xs font-black truncate">{language === 'hi' ? 'रैंक प्रेडिक्टर' : 'Rank Predictor'}</div>
                <div className="text-[10px] text-indigo-200 truncate">{language === 'hi' ? 'बेल कर्व AIR अनुमान' : 'Bell Curve AIR'}</div>
              </div>
            </button>

            {/* 3. Salary Calculator Button */}
            <button
              type="button"
              onClick={onOpenSalaryCalc}
              className="p-2.5 bg-gradient-to-r from-emerald-600/90 to-teal-700/90 hover:from-emerald-500 hover:to-teal-600 text-white rounded-xl border border-emerald-400/30 text-left transition-all shadow-sm hover:scale-[1.02] cursor-pointer flex items-center gap-2.5"
            >
              <div className="p-2 bg-emerald-500/20 rounded-lg shrink-0">
                <Calculator className="w-4 h-4 text-emerald-300" />
              </div>
              <div className="min-w-0">
                <div className="text-xs font-black truncate">{language === 'hi' ? 'सैलरी कैलकुलेटर' : 'Salary Calculator'}</div>
                <div className="text-[10px] text-emerald-200 truncate">{language === 'hi' ? '7th Pay In-Hand Pay' : '7th Pay In-Hand Pay'}</div>
              </div>
            </button>

            {/* 4. Cut-Off Predictor Button */}
            <button
              type="button"
              onClick={onOpenCutOffPredictor}
              className="p-2.5 bg-gradient-to-r from-blue-600/90 to-indigo-700/90 hover:from-blue-500 hover:to-indigo-600 text-white rounded-xl border border-blue-400/30 text-left transition-all shadow-sm hover:scale-[1.02] cursor-pointer flex items-center gap-2.5"
            >
              <div className="p-2 bg-blue-500/20 rounded-lg shrink-0">
                <TrendingUp className="w-4 h-4 text-amber-300" />
              </div>
              <div className="min-w-0">
                <div className="text-xs font-black truncate">{language === 'hi' ? 'कट-ऑफ प्रेडिक्टर' : 'Cut-Off Predictor'}</div>
                <div className="text-[10px] text-blue-200 truncate">{language === 'hi' ? 'अंक एवं सेफ जोन जांचें' : 'Marks & Safe Zone'}</div>
              </div>
            </button>

            {/* 5. Syllabus Checklist Button */}
            <button
              type="button"
              onClick={onOpenSyllabusChecklist}
              className="p-2.5 bg-gradient-to-r from-purple-600/90 to-indigo-800/90 hover:from-purple-500 hover:to-indigo-700 text-white rounded-xl border border-purple-400/30 text-left transition-all shadow-sm hover:scale-[1.02] cursor-pointer flex items-center gap-2.5"
            >
              <div className="p-2 bg-purple-500/20 rounded-lg shrink-0">
                <BookOpen className="w-4 h-4 text-purple-300" />
              </div>
              <div className="min-w-0">
                <div className="text-xs font-black truncate">{language === 'hi' ? 'सिलेबस चेकलिस्ट' : 'Syllabus Tracker'}</div>
                <div className="text-[10px] text-purple-200 truncate">{language === 'hi' ? 'विषयवार तैयारी ट्रैकर' : 'Topic Prep Checklist'}</div>
              </div>
            </button>

            {/* 6. Age Calculator Button */}
            <button
              type="button"
              onClick={onOpenAgeCalc}
              className="p-2.5 bg-gradient-to-r from-amber-600/90 to-orange-700/90 hover:from-amber-500 hover:to-orange-600 text-white rounded-xl border border-amber-400/30 text-left transition-all shadow-sm hover:scale-[1.02] cursor-pointer flex items-center gap-2.5"
            >
              <div className="p-2 bg-amber-500/20 rounded-lg shrink-0">
                <Calendar className="w-4 h-4 text-yellow-200" />
              </div>
              <div className="min-w-0">
                <div className="text-xs font-black truncate">{language === 'hi' ? 'आयु कैलकुलेटर' : 'Age Calculator'}</div>
                <div className="text-[10px] text-amber-200 truncate">{language === 'hi' ? 'सरकारी उम्र पात्रता' : 'Govt Job Eligibility'}</div>
              </div>
            </button>
          </div>
        </div>
