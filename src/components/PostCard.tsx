...
import { getPostImage, cleanTitleText, getTopicUnsplashImage, generateH1ImageBanner, generatePostThumbnailResponsiveSet } from '../utils/imageGenerator';
import { WebpImage } from './WebpImage';
import { SafeText, sanitizeAndDecodeText } from '../hooks/useSafeTextRenderer';
import { trackPostImpression, trackPostClick } from '../utils/analytics';
...
export const PostCard = React.memo(function PostCard({
  ...
  const thumbnailSet = generatePostThumbnailResponsiveSet(post, { target: 'card', quality: 75 });

  useEffect(() => {
    trackPostImpression(post, 'card_feed');
  }, [post.id]);

  const postPath = `/${post.category}/${post.slug || post.id}`;

  const handleCardClick = (e: React.MouseEvent) => {
    if (e.ctrlKey || e.metaKey || e.shiftKey) return;
    e.preventDefault();
    trackPostClick(post, 'card_click');
    onClick(post);
  };
  ...
  return (
    <a
      ...
      className="group bg-white rounded-xl border border-slate-200 hover:border-[#0F4C81] shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer overflow-hidden flex flex-col justify-between relative text-inherit no-underline"
    >
      {/* Thumbnail Header Image */}
      <div className="w-full h-44 bg-slate-100 overflow-hidden relative">
        <WebpImage
          src={thumbnailSet.src}
          responsiveSet={thumbnailSet}
