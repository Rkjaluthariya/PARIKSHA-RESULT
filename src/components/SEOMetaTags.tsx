...
export const SEOMetaTags: React.FC<SEOMetaTagsProps> = ({
  ...
  useEffect(() => {
    ...
    // Always preserve exact deep pathname as default canonical to prevent canonical flip-flopping
    ...
    // 1b. Deep Post URL while hydrating/loading
    else if (pathSegments.length >= 2 && !cleanPath.startsWith('/state/') && !cleanPath.startsWith('/tools/')) {
      const slug = pathSegments[1];
      const prettySlugTitle = slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      titleToSet = `${prettySlugTitle} 2026: Apply Online, Notification, Admit Card & Result | Pariksha Result`;
      descriptionToSet = `Get complete details, eligibility, important dates, and direct links for ${prettySlugTitle} recruitment on Pariksha Result.`;
      keywordsToSet = `${prettySlugTitle}, Sarkari Result 2026, Online Form, Admit Card, Cut Off, Answer Key`;
    ...
    // 3. Category Page Active (Only when single segment e.g. /latest-jobs, /results)
    else if (pathSegments.length === 1 && activeCategory && activeCategory !== 'all' && CATEGORY_SEO_DATA[activeCategory]) {
