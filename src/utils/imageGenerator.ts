...
import { toWebpUrl, generateResponsiveImageSet, getResponsiveSizes } from './webpConverter';
...
export function getPostRawImageUrl(post: any): string {
  if (!post) {
    return CATEGORY_IMAGE_MAP.news_updates[0];
  }
  ...
  return validUrl || getTopicUnsplashImage(post.title || '', post.category || '', post.id || post.slug || '');
}

export function getPostImage(post: any, options: { width?: number; quality?: number } = {}): string {
  const rawUrl = getPostRawImageUrl(post);
  return toWebpUrl(rawUrl, { width: options.width || 800, quality: options.quality || 75 });
}

/**
 * Generates an SEO-optimized responsive image set (WebP srcset, Fallback srcset, sizes, dimensions, alt text)
 * for any post thumbnail or featured image.
 */
export function generatePostThumbnailResponsiveSet(
  post: any,
  options: {
    target?: 'card' | 'banner' | 'modal' | 'table';
    quality?: number;
    widths?: number[];
  ...
  const rawUrl = getPostRawImageUrl(post);
  const cleanTitle = cleanTitleText(post?.title || '') || post?.organization || 'Pariksha Result 2026';
  const altText = post?.imageAltText || `${cleanTitle} Official Notification & Updates`;
  const target = options.target || 'card';

  return generateResponsiveImageSet(rawUrl, altText, {
    quality: options.quality || 75,
    widths: options.widths,
    aspectRatio: options.aspectRatio || (target === 'banner' || target === 'modal' ? '16:9' : '16:9'),
    sizes: getResponsiveSizes(target),
    defaultWidth: target === 'card' ? 600 : target === 'table' ? 320 : 1200,
  });
}
