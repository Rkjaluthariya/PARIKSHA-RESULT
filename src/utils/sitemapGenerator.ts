...
/**
 * Canonical URL & Sitemap Verification Result
 */
export interface CanonicalSitemapCheckResult {
  isValid: boolean;
  canonicalUrl: string;
  category: string;
  slug: string;
...
/**
 * Validates a single post's canonical URL and sitemap mapping
 */
export function verifyPostCanonicalAndSitemap(
  post: any,
  baseUrl: string = BASE_URL,
  existingPostsList?: any[]
): CanonicalSitemapCheckResult {
  const issues: string[] = [];
  const category = (post?.category || 'latest-jobs').trim();
  const slug = (post?.slug || post?.id || '').trim();

  if (!slug) {
    issues.push('Missing unique slug or post identifier');
  }

  // Canonical format check: must be https://pariksha-result.vercel.app/<category>/<slug>
  const canonicalUrl = `${baseUrl}/${category}/${slug}`;

  if (!/^https?:\/\/[a-zA-Z0-9.-]+\/[a-zA-Z0-9_-]+\/[a-zA-Z0-9_-]+$/.test(canonicalUrl)) {
    issues.push('Canonical URL contains invalid characters or malformed path structure');
  }

  if (post?.openGraph?.url && post.openGraph.url !== canonicalUrl) {
    issues.push(`OpenGraph URL mismatch: Found "${post.openGraph.url}", expected canonical "${canonicalUrl}"`);
  }

  // Check presence in generated sitemap
  const targetList = existingPostsList || INITIAL_POSTS;
  const matchInList = targetList.some((p: any) => {
    const pCat = (p?.category || 'latest-jobs').trim();
    const pSlug = (p?.slug || p?.id || '').trim();
    return pCat === category && pSlug === slug;
  });

  const isInSitemap = matchInList || post?.id !== undefined;
...
/**
 * Validate all posts in bulk to detect canonical or sitemap discrepancies
 */
export function verifyAllPostsCanonicalAndSitemaps(
  postsList: any[],
  baseUrl: string = BASE_URL
): {
  totalChecked: number;
  ...
  const results = list.map((post) => {
    const check = verifyPostCanonicalAndSitemap(post, baseUrl, list);
    return {
      id: post?.id || 'unknown',
